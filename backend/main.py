...
async def confirm_payment(request: PaymentRequest, confirmation: UserConfirmation):
    """
    Handle user confirmation/cancellation.
    For HIGH risk, user can confirm after verification step.
    For MEDIUM risk, user can confirm directly.
    """
    if confirmation.confirmed:
        # Re-run analysis to get final decision
        analysis = await analyze_payment(request)

        if analysis.category in ["low", "medium", "high"]:
            # Allowed to proceed
            outcome = "completed"
        else:
